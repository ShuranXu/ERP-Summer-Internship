...
@synthesize stored_data = _stored_data;
@synthesize DirPath = _DirPath;
@synthesize FilePath = _FilePath;
@synthesize preset_row = _preset_row;
...
-(void)Read_Score_From_Local_File
{
    ...
    self.DirPath2 = [documentsDirPath stringByAppendingPathComponent:@"Score1"];//Create an new path under "Document" directory.
    self.FilePath2 = [_DirPath2 stringByAppendingPathComponent:@"score1.txt"];//Create a path for the text file for the saving data under "Document" directory.
    NSError* error;
...
-(void)Read_Data_From_Local_File
{
    NSArray* paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);//Creates a list of directory search paths.
    NSString* documentsDirPath = [paths objectAtIndex:0];//Obtain the path for Documents directory.
    self.DirPath = [documentsDirPath stringByAppendingPathComponent:@"Names"];//Create an new path under "Document" directory.
    self.FilePath = [_DirPath stringByAppendingPathComponent:@"name.txt"];//Create a path for the text file for the saving data under "Document" directory.
    NSError* error;
    //Read data from the local file: 
...
//如果没有名字的输入，就默认用最近刚添加的:

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    
    // Configure the cell...
    static NSString* reusedIdentifier=@"TableViewCell";
    [self.tableView registerClass:[TableViewCell class] forCellReuseIdentifier:reusedIdentifier];//Registers a class for use in creating new table cells.
  ...
  //  TableViewCell* cell = (TableViewCell*)[tableView dequeueReusableCellWithIdentifier:reusedIdentifier];
    ...
    if((!self.empty_name)&&(!self.empty_score)) //The user both types name and plays games only for the 1st time:
    {
        ...
        if(indexPath.row <= [self.stored_data count])//&&(indexPath.row>0))
        {
